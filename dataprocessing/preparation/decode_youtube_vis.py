...
class YoutubeVISDecoder:
    ...
    def decode_annotations(self, dest_dir):
        ...
        for video_id in tqdm(self.video_files.keys(), desc="decoding annotations", unit="video"): #TODO make mp
            h, w = self.video_sizes[video_id]

            for i, segms in enumerate(self.annotations[video_id].values()):
                ...
                mask_path = os.path.join(dest_dir, os.path.splitext(self.video_files[video_id][i])[0] + "_mask.png")
                if not io_utils.write_img(mask, mask_path):
                    print("could not write mask", mask_path)
                self.segm_files[video_id].append(mask_path)
